package com.thedasmc.stocks2.common;

public class Constants {

    public static final String API_URL = "https://api.thedasmc.com";
//    public static final String API_URL = "http://localhost:8080";
    public static final String TOKEN_PLACEHOLDER = "%token%";

    public static final String POSITIVE_SHARE_LIMITS_CONDITION = "share-limits";

    //Portfolio Inventory constants
    public static final int[] PORTFOLIO_STOCK_SLOTS = {};
    public static final int PORTFOLIO_STOCKS = 20;
    public static final int PORTFOLIO_SIZE = 45;
    public static final int PORTFOLIO_PREVIOUS_BUTTON = PORTFOLIO_SIZE - 9;
    public static final int PORTFOLIO_NEXT_BUTTON = PORTFOLIO_SIZE - 1;
    public static final int PORTFOLIO_CLOSE_BUTTON = PORTFOLIO_SIZE - 5;

}
